...
export default class DocComponent extends BaseComponent{
    
    /** 1-一级分类，2-二级分类，3-章 ,4-节*/
    level: number = 1;
    /** 从1级分类->2级分类的1级分类的id,即2级分类的pid */
    pid_for2: number|any = '';
    /** 从2级分类->3级分类的2级分类的id,即3级分类的pid */
    pid_for3: number|any = '';
    /** 从章->节的章的id,即节的pid */    
    pid_for4: number|any = '';
    /** 搜索名称 */
    ...
    /** 获取实体api前缀 */
    get APIPrefix() {
        return this.level === 4?'/docContent':'/docClass';
    }
    ...
    selectEntities(pid: number) {
        this.httpRequest(this.http.get(this.APIPrefix + '/b/select/list?pid='+pid), {
            success: (data: any) => {
                this.list = this.level!==4
                ?data.docClasses:data.docContents;
            }
        });
    }
    ...
    selectOneByPrimaryKey(id: number) {
        ...
        this.httpRequest(this.http.get(this.APIPrefix + '/b/select/one?id='+id), {
            success: (data: any) => {
                this.entity = this.level!==4
                ?data.docClass:data.docContent;
                this.window.open();
            }
        });
    }
