/*
* Filename: d:\frontend\vue\nuxt-ssr\pages\wenyun\detail\_id.ts
* Path: d:\frontend\vue\nuxt-ssr
...
export default class DetailComponent extends BaseComponent {
  ...
  /** 获取楼层作者角色 */
  get roles() {
    if (!this.wy.user || !this.wy.user.roles) return [];
    return this.wy.user.roles;
  }
  ...
  async init(id: number) {
    ...
    this.topReplys = [];
    this.id = id;
    this.block = this.localStorage.getItem('topic-wenyun');
    // 查询话题内容详情
    await this.getWYDetail();
    // 查询话题内容评论
   ...
   * @pran type 0 - 对问题回复，1 - 对1级回复或2级及以上回复
   */
