...
@Component(options)
export default class ZhixingComponent extends BaseComponent {
    /** 关注分类 */
    classify: any = null;
     ...
     */
    getClassify() {
        this.httpRequest(this.http.get('/topic/f/wenyun/select/list?max=7'), {
            success: (data: any) => {
                const topics = data.topics;
                if (topics.concern) {
                    topics.concern.unshift({id: -1, name:'全站'});
                } 
     ...
     */
    getContentList() {
        const qstr = '?wt=1&tid='+this.topicId
    ...
    updateConcern(ids: any) {
        ...
        this.httpRequest(this.http.post('/concern/f/wenyun/batch/insert', ids), {
            error: () => {
     ...
     */
    toDetail(v: any) {
        this.httpRequest(this.http.get('/topic/f/select/one?id='+v.tid), {
            success: (data: any) => {
                this.localStorage.setItem('topic-wenyun', data.topic);
                this.$router.push('/wenyun/detail/?id='+v.id);
            }
        });
    }
  ...
  getRecommendTopicContents() {
    this.httpRequest(this.http.get('/topicContent/f/select/recommend/list?wt=1&pageSize=10'), {
      success: (data: any) => {
